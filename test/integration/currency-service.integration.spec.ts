...
import {
  Currency,
  CurrencyRepository,
} from '../../src/repository/currency.repository';
...
describe('Currency Service Integration Test', () => {
  ...
  it('should return values from file', async () => {
    ...
    when(configMock.getCurrenciesPath()).thenReturn(
      join(__filename, '..', '__mocks__', 'valid-currencies.json')
    );
    const currencyService = new CurrencyService(
      new CurrencyRepository(instance(configMock))
    );
  ...
  it('should throw when invalid file is provided', async () => {
    // given
    when(configMock.getCurrenciesPath()).thenReturn(
      join(__filename, '..', '__mocks__', 'invalid-currencies.json')
    );
    try {
